...
export function FreezeHoldPrintDocument({ data, printable = true }: { data: FreezeHoldPrintPackage; printable?: boolean }) {
  return <article id={printable ? 'freeze-hold-print-report' : 'freeze-hold-evidence-report'} aria-label="CHAINTRACE evidence report">
    ...
    <Section title="8. Custodial Endpoint Intelligence">{data.custodialEndpoints.length ? data.custodialEndpoints.map(endpoint => <div className="report-entry" key={endpoint.address}><dl><Field label="Wallet address" value={endpoint.address} /><Field label="Attribution status / confidence" value={endpoint.status} /></dl><p>{endpoint.attribution}</p>{endpoint.status !== 'VERIFIED CUSTODIAL ENDPOINT' && <p>No verified custodial attribution is currently available.</p>}</div>) : <p>No verified custodial attribution is currently available.</p>}</Section>
    ...
    <Section title="11. Authorized Intervention"><dl><Field label="Reason for request" value={data.reason} /><Field label="Investigator notes" value={data.analystNotes} /><Field label="Requested intervention" value={data.requestedIntervention} /><Field label="Priority" value={data.priority} /><Field label="Target entity (analyst-entered)" value={data.targetEntity} /><Field label="CHAINTRACE internal status" value={data.status} /></dl><p>Prepared for Authorized Escalation describes internal package preparation, not external delivery.</p><p>{data.delivery}</p>{data.targetDetails && <><h3>Target Entity</h3><p>Analyst-entered - not blockchain-verified attribution.</p><dl><Field label="Entity type" value={data.targetDetails.entityType} /><Field label="Analyst attribution assessment" value={data.targetDetails.attributionStatus} /><Field label="Supporting source/reference" value={data.targetDetails.attributionReference} /><Field label="Contact / reference" value={data.targetDetails.contactReference} /></dl></>}<h3>External Response Status</h3>{data.externalResponses?.length ? data.externalResponses.map((response, index) => <div className="report-entry" key={index}><p>Analyst recorded; not independently verified by CHAINTRACE.</p><dl><Field label="External status" value={response.status} /><Field label="External Reference ID" value={response.referenceId} /><Field label="Responding entity" value={response.entity} /><Field label="Response timestamp (UTC)" value={response.respondedAt} /><Field label="Recorded timestamp (UTC)" value={response.recordedAt} /><Field label="Analyst confirmation" value={response.confirmed ? 'Confirmed by analyst as based on an external response' : 'Not confirmed'} /><Field label="Response notes" value={response.notes} /></dl></div>) : <p>No external response recorded.</p>}<p>External submission is performed through an authorized channel outside CHAINTRACE unless an approved integration is configured.</p></Section>
