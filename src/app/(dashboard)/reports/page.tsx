...
import { createEvidencePayload, INTEGRITY_NOTICE } from '@/lib/evidence-integrity';
...
export default function Reports() {
  ...
  const evidencePayload = selectedCase && !loading && !error ? createEvidencePayload('case-report', reportPayload(selectedCase)) : null;
  const integrity = useEvidenceIntegrity(evidencePayload, { caseId: selectedCase?.id, caseCode: selectedCase?.case_code });

  function exportJson() {
    if (!selectedCase || !integrity.record || !evidencePayload) return;
    downloadFile(`${safeFilePart(selectedCase.case_code)}-report.json`, `${JSON.stringify({ ...reportPayload(selectedCase), integrity: integrity.record }, null, 2)}\n`, 'application/json');
  }
